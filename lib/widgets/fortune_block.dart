...
class FortuneBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final DaeunPeriod? sel = hasPeriods
        ? periods[selIdx.clamp(0, periods.length - 1)]
        : null;

    return Padding(
      padding: const EdgeInsets.only(bottom: 16), //화면 밑 여백
      child: Column(
        ...
        children: [
          ...
          if (sel != null) ...[
            Center(
              child: Text(
                '대운수 : ${daeun.startAge} (${daeun.isForward ? "순행" : "역행"})',
  ...
  String _monthGanZhi(
    ...
    final list = manse.entries
        .where(
          (e) => e.key.startsWith('$year${month.toString().padLeft(2, '0')}'),
        )
        .map((e) => e.value)
        .toList();
...
class _Triplet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final BorderSide side = selected
        ? const BorderSide(color: Colors.black, width: 2) // 선택 = 검정 2px
        : const BorderSide(color: Colors.white, width: 1); // 비선택 = 흰색 1px(착시)
    ...
    TextStyle labelStyle = AppTextStyles.body.copyWith(
